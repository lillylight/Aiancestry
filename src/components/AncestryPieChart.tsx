...
export default function AncestryPieChart({ data, onChartReady }: AncestryPieChartProps) {
  ...
  const options = {
    ...
    animation: {
      ...
      duration: 1000,
      onComplete: function() {
        // Another capture attempt when animation completes
        if (chartRef.current && onChartReady && !capturedRef.current) {
          setTimeout(async () => {
            try {
              if (!capturedRef.current) { // Double check
                const chart = chartRef.current;
                if (chart && chart.canvas) {
                  const canvas = chart.canvas as HTMLCanvasElement;
                  const dataUrl = canvas.toDataURL('image/png', 1.0);
                  if (dataUrl && dataUrl.length > 100) {
                    capturedRef.current = true; // Mark as captured
                    console.log('Chart captured on animation complete');
                    onChartReady(dataUrl);
              ...
              console.error('Error in animation complete capture:', error);
            }
          }, 100);
        }
      }
